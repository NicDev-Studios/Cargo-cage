use cage_core::{
    CageError, CageResult, is_allowed_sandbox_environment_name, is_sensitive_environment_name,
};
use landlock::{
...
pub const LAUNCHER_CONTEXT_DESTINATION: &str = "/run/cargo-cage-launcher-context";
pub const LAUNCHER_CONTEXT_CONTENT: &[u8] = b"cargo-cage launcher context v1\n";
...
const INHERIT_ENVIRONMENT_ARGS: &str = "--landlock-inherit-environment";
...
fn validate_launcher_path(value: &OsStr, label: &str) -> CageResult<PathBuf> {
    ...
    if path == Path::new("/") {
        return Err(CageError::policy(
            path.display().to_string(),
            format!("{label} must not be the filesystem root"),
            "use the narrow canonical sandbox destination generated by cargo-cage",
        ));
    }
    Ok(path)
}
...
pub fn run_internal_launcher(args: &[OsString]) -> CageResult<i32> {
    ...
    for (name, value) in env::vars_os() {
        if plan.inherit_environment
            || (is_allowed_sandbox_environment_name(&name) && !is_sensitive_environment_name(&name))
        {
            command.env(name, value);
        }
    }
    let child = command.status().map_err(|error| {
        CageError::io(
            format!(
...
fn verify_launcher_context() -> CageResult<()> {
    let metadata = std::fs::symlink_metadata(LAUNCHER_CONTEXT_DESTINATION).map_err(|error| {
        CageError::sandbox_setup(
            LAUNCHER_CONTEXT_DESTINATION,
            "the Landlock launcher must be started from cargo-cage's Bubblewrap context",
            "invoke cargo-cage directly and do not execute the internal launcher binary",
            format!("the Bubblewrap launcher-context marker is unavailable: {error}"),
        )
    })?;
    if !metadata.is_file() || metadata.len() != LAUNCHER_CONTEXT_CONTENT.len() as u64 {
        return Err(CageError::sandbox_setup(
            LAUNCHER_CONTEXT_DESTINATION,
            "the launcher-context marker must be the expected regular file",
            "invoke cargo-cage directly and do not execute the internal launcher binary",
            "the marker has the wrong type or length",
        ));
    }
    let content = std::fs::read(LAUNCHER_CONTEXT_DESTINATION).map_err(|error| {
        CageError::sandbox_setup(
            LAUNCHER_CONTEXT_DESTINATION,
            "the launcher-context marker must be readable before Landlock is applied",
            "retry with a working Bubblewrap file setup",
            format!("could not read the launcher-context marker: {error}"),
        )
    })?;
    if content != LAUNCHER_CONTEXT_CONTENT {
        return Err(CageError::sandbox_setup(
            LAUNCHER_CONTEXT_DESTINATION,
            "the launcher-context marker must match cargo-cage's fixed context",
            "invoke cargo-cage directly and do not execute the internal launcher binary",
            "the marker content did not match",
        ));
    }
    Ok(())
}
...
fn landlock_setup_error(detail: impl Into<String>, remedy: impl Into<String>) -> CageError {
    CageError::sandbox_setup(
        "Landlock policy",
        "every required Landlock restriction must be enforced before the child starts",
        remedy,
        detail,
    )
}
...
mod tests {
    ...
    #[test]
    fn launcher_rejects_filesystem_root_policy_paths() {
        let error = LauncherPlan::parse(&[
            OsString::from(WRITE_ARGS),
            OsString::from("/"),
            OsString::from("--"),
            OsString::from("/bin/sh"),
        ])
        .expect_err("root policy path");
        let text = error.to_string();
        assert!(text.contains("filesystem root"), "{text}");
